...
import type { DashboardWidget, PersonalRecordWidgetConfig, FilterCondition } from "../../types";
...
export default function PersonalRecordConfig({ widget }: Props) {
  ...
  const existing = widget?.config.type === "personal_record" ? widget.config : null;

  const [metric, setMetric] = useState(existing?.metric ?? PERSONAL_RECORD_METRICS[0].key);
  ...
  const handleSave = () => {
    const config: PersonalRecordWidgetConfig = {
    ...
    if (widget) {
      ...
      addWidget("personal_record", config);
    }
  };
